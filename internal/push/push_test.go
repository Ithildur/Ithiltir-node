...
func TestStartPushAgentRetriesStaticWhileMetricsContinue(t *testing.T) {
	...
	go func() {
		errCh <- start(ctx, host, port, "secret", 10*time.Millisecond, snapshotter, false, false, nil)
	}()
...
func TestStartPushAgentStopsAfterConsecutive422(t *testing.T) {
	var metricsPosts atomic.Int32
	...
	err := start(ctx, host, port, "secret", 10*time.Millisecond, snapshotter, false, false, nil)
	if err == nil || err.Error() != "push stopped after consecutive 422 responses" {
		t.Fatalf("start() error = %v, want consecutive 422 stop", err)
	}
...
func TestStartPushAgentRequireHTTPSDoesNotFallback(t *testing.T) {
	...
	err := start(ctx, host, port, "secret", 10*time.Millisecond, snapshotter, false, true, nil)
